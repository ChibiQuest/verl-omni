...
"""Build a tiny Qwen-Image checkpoint for smoke tests fully offline.

Usage:
    python tests/special_e2e/build_qwen_image_tiny_random.py \
        --output-dir ~/models/tiny-random/Qwen-Image
"""
...
DEFAULT_OUTPUT_DIR = os.path.expanduser("~/models/tiny-random/Qwen-Image")
...
def _build_tiny_chatml_tokenizer(*, vocab_size: int = 2048) -> Qwen2TokenizerFast:
    """Build a tiny Qwen tokenizer with the special tokens used by the text encoder."""
    ...
    tokenizer.train_from_iterator(
        [
            "A beautiful landscape with mountains, a river, and soft golden sunlight.",
            "A red circle on a white background.",
            "Generate a blue square on a clean white background.",
...
def build(
    ...
    *,
    ...
    """Construct and save a tiny random-weight Qwen-Image checkpoint."""
...
def ensure_tiny_qwen_image_checkpoint(
    output_dir: str,
    *,
    hidden_size: int = 16,
    seed: int = 42,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a tiny Qwen-Image checkpoint offline (random weights).",
    )
    ...
    output_dir = ensure_tiny_qwen_image_checkpoint(
        args.output_dir,
    ...
    print(f"Tiny Qwen-Image checkpoint ready at {output_dir}")
